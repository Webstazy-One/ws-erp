        ...
        .then((data) => {
            ...
            Item.findById(data[0].itemId)
                .then(itemData => {
                    ...
                    Promo.find({ appliedto: data[0].itemId, startDate: { $lt: nDate }, endDate: { $gte: nDate } })
                        .then(promoData => {
                            console.log(data[0].itemId)
                            if (!promoData[0] == null) {
...
exports.getDetailsOfProductCreation = (req, res) => {
        ...
        .then((data) => {
            data.forEach(sale => {
                ...
                Item.findById(sale.itemId)
                    .then(itemData => {
                        Promo.find({ appliedto: req.params.id, startDate: { $lt: nDate }, endDate: { $gte: nDate } }
